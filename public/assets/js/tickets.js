document.addEventListener('DOMContentLoaded', () => {
  ...
  function humanStatus(s) { return (s || '').replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase()); }
  ...
  function currentFilter() { return (statusFilter && statusFilter.value) || 'all'; }
  ...
  function render() {
    ...
    if (items.length === 0) {
      ...
      emptyText.textContent = filter === 'all' ? 'Get started by creating a new ticket.' : `No ${filter.replace('_', ' ')} tickets found.`;
      return;
    }
      ...
      card.innerHTML = `
        <div class="px-4 py-5 sm:p-6">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-medium leading-6 text-gray-900">${escapeHtml(ticket.title)}</h3>
            <div class="flex space-x-2">
              <button data-edit="${ticket.id}" class="text-gray-400 hover:text-blue-500" title="Edit ticket">
                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                </svg>
              </button>
              <button data-del="${ticket.id}" class="text-gray-400 hover:text-red-500" title="Delete ticket">
                <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
          ...
          ${ticket.description ? `<p class="mt-2 text-sm text-gray-600 line-clamp-3">${escapeHtml(ticket.description)}</p>` : ''}
          <div class="mt-4 flex flex-wrap gap-2">
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_CLASS[ticket.status] || STATUS_CLASS.closed}">${humanStatus(ticket.status)}</span>
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${PRIORITY_CLASS[ticket.priority] || PRIORITY_CLASS.medium}">${cap(ticket.priority)}</span>
          </div>
          <div class="mt-4 text-xs text-gray-500">Created: ${formatDate(ticket.createdAt)}</div>
        </div>
      `;
  ...
  emptyCreateBtn && emptyCreateBtn.addEventListener('click', () => {
    editingId = null;
    form && form.reset();
    openModal('Create New Ticket');
    if (submitBtn) submitBtn.textContent = 'Create Ticket';
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (idEdit) { handleEdit(idEdit); }
    if (idDel) { handleDelete(idDel); }
  });
  ...
  form && form.addEventListener('submit', (e) => {
    ...
    if (title.length < 3) {
      if (errorTitle) { errorTitle.textContent = 'Title must be at least 3 characters long.'; errorTitle.classList.remove('hidden'); }
      return;
    } else {
      if (errorTitle) { errorTitle.classList.add('hidden'); }
    }
    ...
    if (description && description.length > 2000) {
      toast('Description is too long (max 2000 characters).', '#ef4444');
      return;
    }
    ...
    if (editingId) {
      ...
      if (idx !== -1) {
        tickets[idx] = { ...tickets[idx], title, description, status, priority };
        saveTickets(tickets);
  ...
  function toast(msg, color) {
    if (window.Toastify) {
      Toastify({ text: msg, duration: 2500, gravity: 'top', position: 'right', backgroundColor: color || '#3b82f6' }).showToast();
    }
  }
